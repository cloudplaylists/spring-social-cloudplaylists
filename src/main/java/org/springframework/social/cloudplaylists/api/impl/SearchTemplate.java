...
public class SearchTemplate extends AbstractCloudPlaylistsResourceOperations implements SearchOperations {

	public SearchTemplate(String apiBaseUrl, RestTemplate restTemplate, boolean isAuthorizedForUser) {
	...
	public Page<Media> searchSoundCloud(String q, Pageable pageable) {
		...
		return restTemplate.getForObject(getApiResourceUrl("/soundcloud?q=" + q, pageable), MediaPage.class);
	}

	@Override
	public Page<Media> searchSoundCloud(String q) {
		requireAuthorization();
		return restTemplate.getForObject(getApiResourceUrl("/soundcloud?q=" + q), MediaPage.class);
	}
	...
	public Media resolveMedia(String url, MediaProvider mediaProvider, boolean validate) {
		return restTemplate.getForObject(getApiResourceUrl("/media/" + mediaProvider.providerId().toLowerCase()
				+ "?url=" + url + (validate ? "&validate=true" : "")), Media.class);
	}

	@Override
	public Map<MediaProvider, Media> resolveMedia(String url) {
		return restTemplate.getForObject(getApiResourceUrl("/media?url=" + url), MediaMap.class);
	}
